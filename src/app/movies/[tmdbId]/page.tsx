...
import { getMovieDetails, getSimilarMovies, getMovieCredits, getMovieWatchProviders } from '@/lib/tmdb';
...
export default async function MovieDetailPage({ params }: { params: { tmdbId: string } }) {
  ...
  try {
    const [creditsData, providersData] = await Promise.all([
      getMovieCredits(Number(tmdbId)),
      getMovieWatchProviders(Number(tmdbId)),
    ]);
  ...
  return (
    <>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 md:py-10">
        <Breadcrumbs items={[{ label: 'Home', href: '/' }, { label: 'Movies', href: '/movies' }, { label: movie.title }]} />
        <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-start">
          ...
          {/* Main Content — order-1 on mobile so it shows above the sidebar */}
          ...
          <div className="mt-12">
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 -mx-4 px-4">
              {similarMovies.map((m: any) => {
                ...
                return (
                  <div key={m.id} className="min-w-[160px] sm:min-w-[200px] max-w-[180px] sm:max-w-[220px]">
